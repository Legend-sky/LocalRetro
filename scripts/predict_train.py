...
def main(args):
    ...
    args['result_path'] = '../outputs/raw_prediction_train/%s' % model_name.replace('.pth', '.txt')
    mkdir_p('../outputs')
    mkdir_p('../outputs/raw_prediction_train')
    
    args = init_featurizer(args)
    args = get_site_templates(args)
    model = load_model(args)
    train_loader = load_dataloader(args)
    write_edits(args, model, train_loader)
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file', default='train', help='predict file')
